...
import { getClasses, getImports, Pattern, Query, StructureType } from '@mutates/core';

export function getNgModules(
  pattern: Pattern,
  query?: Query<Omit<StructureType<ClassDeclaration>, 'kind'>>,
): ClassDeclaration[] {
  return getClasses(pattern, query).filter((declaration) => !!declaration.getDecorator('NgModule'));
}
